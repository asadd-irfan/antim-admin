...
import { GetAllFunders, AddNotification, DeleteFunder } from '../../store/actions/admin.actions';
...
export class FundersComponent implements OnInit {
  ...
  displayedColumns: string[] = ['name', 'TotalReq', 'OngoingReq', 'status', 'actions'];
  ...
  showRequests = false;
  funderName: any;
  funderAllRequestsData: any;
  funderID: any;
  funderRequestType = 'All Requests'
  requestTypes: any[] = [
    {
      type: 'All Requests'
    },
  ...
  getAllFunders() {
    return new Promise((resolve, reject) => {
      this.getState.subscribe((state) => {
        // console.log('funders in store:', state)
        if (state.AllFunders.length == 0) {
          this.store.dispatch(new GetAllFunders());
        }
        this.fundersData = state.AllFunders;
        if (state.AllFunders.length > 0) {
          resolve();
        }
      });
  ...
  ngOnInit() {
    this.getAllFunders().then(e => {
      if (this.fundersData.length > 0) {
        this.showUserMessage = false;
        AllFunders.length = 0;
        this.fundersData.map(element => {
          let funder = Object.assign({}, element, {
            status: this.userStatus[element.status].type
          })
          AllFunders.push(funder);
        });
        this.dataSource = new MatTableDataSource<any>(AllFunders);
        this.dataSource.paginator = this.paginator;
      } else {
    ...
    if (this.fundersData && this.fundersData.length == 0) {
  ...
  openUserDetails(row) {
    this.userID = row.id;
    this.router.navigate(['funders', this.userID]);
  }
  Search(value) {
  ...
  openRequests(row) {
    let id = row.id;
    this.spinner.show();
    this.adminApiService.getFunderRequestsByUserId(id).subscribe(res => {
      this.spinner.hide();
      console.log(res);
      if (res.message) {
        ...
        this.funderAllRequestsData = res.result;
        AllRequests.length = 0;
        this.funderAllRequestsData.forEach(element => {
  ...
  openProductDetails(row) {
    localStorage.setItem('funder-page', 'go-to-funder-page');
  ...
  onChange(deviceValue) {
    ...
    this.funderRequestType = deviceValue;
    if (deviceValue == 'All Requests') {
      this.dataSourceRequests.filter = '';
      this.funderRequestType = 'All Requests';
    }
  ...
  deleteFunder() {
    ...
    this.adminApiService.deleteUser({
      ...
      this.store.dispatch(new DeleteFunder({ id: this.funderID }));
      this.spinner.hide();
      this.ngOnInit();
      this.modalService.dismissAll();
      this.showSuccessToast('OK!!', 'User  ' + this.funderName + '  has been deleted Successfully.', 'success');
      console.log(res);
      this.reason = '';
    }, err => {
      this.spinner.hide();
  ...
  downloadFunderData() {
    this.spinner.show();
    this.adminApiService.getUserReports('funder', this.fromDate, this.toDate).subscribe(res => {
      this.spinner.hide();
      console.log(res);
